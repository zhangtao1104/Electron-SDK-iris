...
void NodeIrisRtcChannel::CallApi(
  ...
  try {
    _ret = _channel->_iris_channel.get()->CallApi((ApiTypeChannel)_apiType,
                                                  _parameter.c_str(), _result);
  } catch (std::exception &e) {
    _channel->OnApiError(e.what());
    LOG_F(
        INFO,
        "NodeIrisRtcChannel::CallApi apiType: %d, parameter: %s, exception: %s",
...
void NodeIrisRtcChannel::CallApiWithBuffer(
  ...
  try {
    ...
    case ApiTypeChannel::kChannelSendStreamMessage: {
      ...
      _ret = _channel->_iris_channel.get()->CallApi(
